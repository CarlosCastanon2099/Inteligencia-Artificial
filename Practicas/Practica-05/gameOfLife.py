...
n_celdas_x = 50
n_celdas_y = 50
celda_tam = 15 # Tamaño de las celdas de la cuadricula, al modificar este valor se modifica el tamaño de las celdas de la cuadricula
...
# Configuraramos la pantalla en la que desplegaremos la cuadricula y todo lo demas lol
...
# esta variable claramente existe por que sabiamos que si no existia podiamos llegar a la generacion 1000 
# en muy poco tiempo y no por que al probar este programa llegamos al millon a la velocidad de la luz XD
...
def dibujar(tablero):
    ...
    for y in range(n_celdas_y):
        for x in range(n_celdas_x):
            if tablero[x, y]:
                pygame.draw.rect(pantalla, CELESTE, (x * celda_tam, y * celda_tam, celda_tam, celda_tam)) # Dibujamos las celulas vivas
            pygame.draw.rect(pantalla, NEGRO, (x * celda_tam, y * celda_tam, celda_tam, celda_tam), 1)    # Dibujamos la cuadricula 
...
while ejecutando:
    ...
    if jugando:
        ...
        tablero_siguiente = np.copy(tablero) # Creamos una copia del tablero actual para poder modificarlo sin afectar el tablero original (esta copia 
                                        # solo se guarda en codigo y no se dibuja o se muestra en pantalla)
        for x in range(n_celdas_x):
            for y in range(n_celdas_y):
                vecinos = np.sum(tablero[(x-1):(x+2), (y-1):(y+2)]) - tablero[x, y] # Contamos los vecinos vivos de cada celula (celda)
                if tablero[x, y] and (vecinos < 2 or vecinos > 3): # En caso de que la celula este viva y tenga menos de 2 o mas de 3 vecinos vivos, 
                                                                   # la celula muere brutalmente 😢
